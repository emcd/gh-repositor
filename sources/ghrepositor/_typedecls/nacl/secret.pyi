...
class Aead(encoding.Encodable, StringFixer):
    ...
    def encrypt(self, plaintext: bytes, aad: bytes = ..., nonce: Optional[bytes] = ..., encoder: encoding.Encoder = ...) -> EncryptedMessage:
        """
        Encrypts the plaintext message using the given `nonce` (or generates
        one randomly if omitted) and returns the ciphertext encoded with the
        encoder.
    ...
    def decrypt(self, ciphertext: bytes, aad: bytes = ..., nonce: Optional[bytes] = ..., encoder: encoding.Encoder = ...) -> bytes:
        """
        Decrypts the ciphertext using the `nonce` (explicitly, when passed as a
        parameter or implicitly, when omitted, as part of the ciphertext) and
        returns the plaintext message.
